...
- **macOS:** Quartz events and Accessibility focus checks. Before arming, the helper requests Electron’s accessibility tree and resolves the field through the focused application. If the tree remains unavailable, the status includes the AX error code; set **Editor: Accessibility Support** to **on** and re-arm. Grant the Python helper Accessibility permission manually if its status requests it. The extension does not open System Settings or request permission automatically.

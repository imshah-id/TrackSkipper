...
- **Scrolling:** playback scrolls smoothly through the code toward each change and keeps the current line visible while typing or deleting, including in short editors and with VM system input enabled. Pause to browse a fixed position manually; **Follow playback** returns to the active line. Reduced motion disables the visual animation.

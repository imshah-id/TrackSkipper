...
async function main() {
  ...
  try {
    ...
    await settle("window.scrollFrame={firstLine:0,viewportLine:0,lines:Array.from({length:120},(_,i)=>'const line'+i+' = '+i+';'),caret:{row:30,column:0}};pushState({...fixturePlayback,frame:scrollFrame,phase:{...fixturePlayback.phase,kind:'type'}})");
    await evaluate('new Promise(resolve=>setTimeout(resolve,320))');
    await settle("pushState({...fixturePlayback,frame:{...scrollFrame,viewportLine:20},phase:{...fixturePlayback.phase,kind:'type'}})");
    const scrollY = () => evaluate("new DOMMatrixReadOnly(getComputedStyle(document.querySelector('#code')).transform).m42");
    const movingY = await scrollY();
    assert.ok(movingY < 0 && movingY > -440, 'auto-scroll animates through intermediate positions');
    await settle("pushState({...fixturePlayback,frame:{...scrollFrame,viewportLine:20,caret:{row:31,column:0}},phase:{...fixturePlayback.phase,kind:'type'}})");
    await evaluate('new Promise(resolve=>setTimeout(resolve,320))');
    assert.equal(await scrollY(), -440, 'typing updates let the scroll finish');
    assert.ok(await evaluate("(()=>{const caret=document.querySelector('.caret').getBoundingClientRect(),view=document.querySelector('#code-scroll').getBoundingClientRect();return caret.top>=view.top&&caret.bottom<=view.bottom})()"), 'the changed code stays visible');
    await screenshot('playback-autoscroll.png');
    await settle("document.querySelector('#code-scroll').dispatchEvent(new WheelEvent('wheel',{deltaY:100,cancelable:true}))");
    assert.equal(await evaluate("commands.filter(command=>command.type==='viewport').at(-1).firstLine"), 26, 'manual scrolling starts at the visible line, not the buffered first row');
    await settle("document.querySelector('#code-scroll').dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowUp',bubbles:true}))");
    assert.equal(await evaluate("commands.filter(command=>command.type==='viewport').at(-1).firstLine"), 19);
    await settle("document.querySelector('#native-enabled').click();pushState({...fixturePlayback,frame:{...scrollFrame,viewportLine:24},phase:{...fixturePlayback.phase,kind:'type'}})");
    await evaluate('new Promise(resolve=>setTimeout(resolve,320))');
    ...
    await settle("document.querySelector('.tab-select').click()");
    assert.equal(await evaluate("commands.filter(command=>command.type==='browse').at(-1).offset"), 100, 'tab label selects its file');
    await settle("document.querySelector('.tab-close').click()");
    assert.equal(await evaluate("commands.filter(command=>command.type==='closeTab').at(-1).offset"), 100, 'close button sends a separate tab command');
    await settle("document.querySelectorAll('.tab')[1].dispatchEvent(new MouseEvent('auxclick',{button:1,bubbles:true,cancelable:true}))");
    assert.equal(await evaluate("commands.filter(command=>command.type==='closeTab').at(-1).offset"), 0, 'middle-click closes the chosen tab');
    await settle("pushState({...fixturePlayback,status:'paused',frame:{firstLine:0,lines:['\\t  nested()', 'plain()'],caret:null},editor:{...fixturePlayback.editor,tabSize:4}})");
    const guides = await evaluate("({width:document.querySelector('.line-content').style.getPropertyValue('--indent-width'),background:getComputedStyle(document.querySelector('.line-content'),'::before').backgroundImage,text:document.querySelector('.line-content').textContent})");
    assert.equal(guides.width, '6ch', 'mixed tabs and spaces use editor tab stops');
    assert.notEqual(guides.background, 'none');
    assert.equal(guides.text, '\t  nested()', 'guides leave source text unchanged');
    const longTabs = Array.from({length:5}, (_, index)=>({offset:index,label:`src/long-directory/component-with-long-name-${index}.ts`}));
    await settle(`pushState({...fixturePlayback,status:'paused',tabs:${JSON.stringify(longTabs)},activePath:${JSON.stringify(longTabs[4].label)}})`);
    assert.equal(await evaluate("document.querySelector('#tabs').scrollWidth > document.querySelector('#tabs').clientWidth"), true);
    assert.ok(await evaluate("document.querySelector('#tabs').scrollLeft"), 'active overflowing tab is revealed');
    await settle("document.querySelector('#tabs').scrollLeft=0;document.querySelector('#tabs').dispatchEvent(new WheelEvent('wheel',{deltaY:100,cancelable:true}))");
    assert.ok(await evaluate("document.querySelector('#tabs').scrollLeft"), 'wheel scrolls overflowing tabs horizontally');
    assert.ok(await evaluate("commands.some(command=>command.type==='viewportSize' && command.rows>0 && command.rows<=120)"));
    await settle('pushState(fixturePlayback)');
    ...
    assert.deepEqual(await evaluate("commands.filter(command=>command.type==='speed').at(-1)"), { type: 'speed', sessionId: 'playing', charactersPerSecond: 50, pointerMultiplier: 1 });
    ...
    await settle("pushState({...fixturePlayback,editor:{...fixturePlayback.editor,fontSize:16,lineHeight:1.5},frame:{...scrollFrame,viewportLine:20}})");
    assert.equal(await scrollY(), -480, 'reduced motion positions the code immediately using the current line height');
    assert.equal(await evaluate("document.querySelector('#code').getAnimations().length"), 0);
    await settle('pushState(fixturePlayback)');
